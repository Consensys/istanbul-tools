...
var _ = Describe("QFS-05: Byzantine Faulty", func() {
	...
	Context("QFS-05-01: F faulty validators", func() {
		const (
			numberOfNormal = 3
			numberOfFaulty = 1
		)
		...
		BeforeEach(func() {
			constellationNetwork = container.NewDefaultConstellationNetwork(dockerNetwork, numberOfNormal+numberOfFaulty)
			Expect(constellationNetwork.Start()).To(BeNil())
			blockchain = container.NewDefaultQuorumBlockchainWithFaulty(dockerNetwork, constellationNetwork, numberOfNormal, numberOfFaulty)
		...
		AfterEach(func() {
			...
			blockchain.Finalize()
			Expect(constellationNetwork.Stop()).To(BeNil())
	...
	Context("QFS-05-01: F+1 faulty validators", func() {
		const (
			numberOfNormal = 2
			numberOfFaulty = 2
		)
